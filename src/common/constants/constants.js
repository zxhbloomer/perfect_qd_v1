/** 模块类型 */
const DICT_MODULE_TYPE = 'module_type'
const DICT_MODULE_TYPE_PAGE = 'p'
const DICT_MODULE_TYPE_MENU = 'm'
const DICT_MODULE_TYPE_CONTENTS = 'c'

/** 隐藏显示类型： 0：显示 1：隐藏 null:全部 */
const DICT_SYS_VISIBLE_TYPE = 'sys_visible_type'
const DICT_SYS_VISIBLE_TYPE_SHOW = '0'
const DICT_SYS_VISIBLE_TYPE_HIDDEN = '1'
const DICT_SYS_VISIBLE_TYPE_ALL = 'null'

/** 按钮类型： */
const DICT_BTN_NAME_TYPE = 'btn_name_type'

export default {
  /** 模块类型 */
  DICT_MODULE_TYPE,
  DICT_MODULE_TYPE_PAGE,
  DICT_MODULE_TYPE_MENU,
  DICT_MODULE_TYPE_CONTENTS,
  /** 隐藏显示类型： 0：显示 1：隐藏 null:全部 */
  DICT_SYS_VISIBLE_TYPE,
  DICT_SYS_VISIBLE_TYPE_SHOW,
  DICT_SYS_VISIBLE_TYPE_HIDDEN,
  DICT_SYS_VISIBLE_TYPE_ALL,
  /** 按钮类型： */
  DICT_BTN_NAME_TYPE
}
